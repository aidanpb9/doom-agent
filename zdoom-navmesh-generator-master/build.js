...
const SectorNavmesh = require('./src/lib/SectorNavmesh');
...
var buildNavMesh = function(LevelName, config, MasterConfig) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        .catch(err => {
            console.log(err);
            // Fallback: build a simple sector-based navmesh from UDMF
            let mapfile = path.join(MasterConfig.wadspath, `${LevelName}.wad`);
            WadLoader.loadWAD(mapfile)
            .then(wad => WadLoader.getTextMap(wad))
            .then(TEXTMAP => UDMFParser.parse(TEXTMAP))
            .then(udmf => {
                let zones = SectorNavmesh.buildFromUDMF(udmf);
                let destJSON = path.join(MasterConfig.meshpath, `${LevelName}.json`);
                fs.writeFileSync(destJSON, Buffer.from(JSON.stringify(zones, null, 2)), { encoding: 'utf-8' });
                let scenes = UDMFtoTHREE.makeScene(udmf);
                let OBJData = UDMFtoTHREE.getOBJ(scenes.navScene);
                let destOBJ = path.join(MasterConfig.meshpath, `navprev.obj`);
                fs.writeFileSync(destOBJ, Buffer.from(OBJData), { encoding: 'utf-8' });
                return resolve(JSON.stringify(scenes.scenePreview.toJSON()));
            })
            .catch(fallbackErr => {
