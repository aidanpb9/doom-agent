...
2. **Seed:** VizDoom exposes `set_seed()` to control world state RNG (enemy positions, item spawns). Python's RNG is seeded separately per episode. See ga design doc for how both are used during evolution.
