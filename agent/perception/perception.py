...
class PerceptionManager:
    ...
    def detect_blocking_barrel(self, labels, screen_shape=None, require_center=True):
        """Detect a barrel-like object centered ahead that can block movement."""
        if not labels:
            return None
        width = None
        height = None
        if screen_shape is not None and len(screen_shape) >= 2:
            height = int(screen_shape[0])
            width = int(screen_shape[1])
        best = None
        best_area = 0
        for lbl in labels:
            ...
            if (
                "barrel" not in name
                and "explosive" not in name
                and "burning barrel" not in name
                and "drum" not in name
            ):
                continue
            area = float(getattr(lbl, "width", 0) * getattr(lbl, "height", 0))
            min_area = 220.0 if require_center else 120.0
            if area < min_area:
                continue
            cx = float(getattr(lbl, "x", 0)) + float(getattr(lbl, "width", 0)) * 0.5
            cy = float(getattr(lbl, "y", 0)) + float(getattr(lbl, "height", 0)) * 0.5
            if require_center and width is not None and height is not None:
                # Require barrel to be close to center to avoid overreacting.
                if abs(cx - (width * 0.5)) > width * 0.14:
                    continue
                # Must be fairly low on screen (nearby obstacle).
                if cy < height * 0.45:
                    continue
            if area > best_area:
                best_area = area
